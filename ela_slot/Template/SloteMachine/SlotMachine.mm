...
//        [self addChild:reels z:5 tag:kReelTag];
        ...
        [self performSelector:@selector(loadingOff) withObject:nil afterDelay:0.2];
...
-(void)createBG
{
    SKSpriteNode *s = [SKSpriteNode spriteNodeWithImageNamed:@"bg_0.png"];
    s.name = kNodeReelsBG;
    s.position = ccp(kWidthScreen/2, kHeightScreen*(IS_IPAD ? 0.435f : 0.52f));
    [self addChildToTopZ:s];
//    [[self getChildByTag:kSpriteSheet] addChild:s z:0 tag: ];
    ...
    SKSpriteNode *s1 = [SKSpriteNode spriteNodeWithImageNamed:@"bg_1.png"];
    s1.position = ccp(kWidthScreen/2, kHeightScreen/2);
    [self addChildToTopZ:s1];
#warning EF, needs tag or name
...
{
    #warning EF
//    [[NSUserDefaults standardUserDefaults] setInteger:[[NSUserDefaults standardUserDefaults] integerForKey:@"spinToAdd"]+1 forKey:@"spinToAdd"];
...
//    if ([[NSUserDefaults standardUserDefaults] integerForKey:@"spinToAdd"] >= spinsToAd.integerValue) {
//        [[NSUserDefaults standardUserDefaults] setInteger:0 forKey:@"spinToAdd"];
//        [Chartboost showInterstitial:CBLocationGameOver];
//        
...
-(void)topMenuType:(int) menuType
{
    ...
    [self addChildToTopZ:TMenu];
    
    SKAction* moveAction = [SKAction moveTo:CGPointZero duration:0.8];
    [TMenu runAction:moveAction];
}
...
{
    Reels *r = (Reels *)[self childNodeWithName:kNodeReels];// [self getChildByTag:kReelTag];
